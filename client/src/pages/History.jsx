...
function History() {
  ...
  useEffect(() => {
    const fetchSearchHistory = async () => {
      ...
      try {
        ...
        setSearchHistory([]); // Ensure you always get an array, even on error
      } finally {
        setLoading(false);
      }
    };
  ...
  <Card key={index} className="bg-neutral-800 text-white border-neutral-700 hover:border-neutral-600 transition-all">
    <div className="p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <Badge variant="secondary" className="bg-neutral-700">
          {food.result && food.result[0] ? food.result[0].calories : 'No data'} kcal
        </Badge>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="bg-neutral-900 p-3 rounded-lg text-center">
          ...
          <p className="text-lg font-semibold">
            {food.result && food.result[0] ? food.result[0].protein_g : 'N/A'}g
          </p>
        </div>
        <div className="bg-neutral-900 p-3 rounded-lg text-center">
          <p className="text-sm text-neutral-400">Carbs</p>
          <p className="text-lg font-semibold">
            {food.result && food.result[0] ? food.result[0].carbohydrates_total_g : 'N/A'}g
          </p>
        </div>
        <div className="bg-neutral-900 p-3 rounded-lg text-center">
          <p className="text-sm text-neutral-400">Fats</p>
          <p className="text-lg font-semibold">
            {food.result && food.result[0] ? food.result[0].fat_total_g : 'N/A'}g
          </p>
        </div>
      </div>
      ...
      <div className="mt-4 pt-4 border-t border-neutral-700">
        <p className="text-sm text-neutral-400">
          Searched on {new Date(food.searchedAt).toLocaleDateString()}
        </p>
      </div>
    </div>
  </Card>
